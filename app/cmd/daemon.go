...
import (
	...
	"github.com/BASChain/go-bas-discover-wanip/app/cmdservice"
...
var daemonCmd = &cobra.Command{
	Use:   "daemon",
	Short: "disw start in backend",
	Long:  `disw start in backend`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		InitCfg()
		config.GetBasDisCfg().Save()

		daemondir := config.GetBASDisHomeDir()
		cntxt := daemon.Context{
			PidFileName: path.Join(daemondir, "disw.pid"),
			PidFilePerm: 0644,
			LogFileName: path.Join(daemondir, "disw.log"),
			LogFilePerm: 0640,
			WorkDir:     daemondir,
			Umask:       027,
			Args:        []string{},
		...
		if d != nil {
			log.Println("disw starting, please check log at:", path.Join(daemondir, "disw.log"))
			return
		}
		defer cntxt.Release()
